#!/bin/bash

# ニコポケCLI 配布パッケージ作成スクリプト

echo "🎮 ニコポケCLI 配布パッケージを作成します..."

# 出力ディレクトリ
DIST_DIR="nikopoke-cli-dist"
rm -rf "$DIST_DIR"
mkdir -p "$DIST_DIR"

# リリースビルド（既にビルド済みの場合はスキップ）
if [ ! -f "target/release/debug-cli" ] || [ ! -f "target/release/battle-cli" ]; then
    echo "📦 リリースビルドを作成中..."
    cargo build --release --bin debug-cli --bin battle-cli
fi

# 実行可能ファイルをコピー
echo "📋 ファイルをコピー中..."
cp target/release/debug-cli "$DIST_DIR/"
cp target/release/battle-cli "$DIST_DIR/"

# データファイルをコピー（実行可能ファイルに埋め込まれているが、念のため）
cp -r data "$DIST_DIR/"

# READMEを作成
cat > "$DIST_DIR/README.md" << 'EOF'
# 🎮 ニコポケ バトルCLI

ニコポケ用のバトルエンジン デバッグツールです。

## 📦 内容物

- `debug-cli` - デバッグCLI（機能が豊富）
- `battle-cli` - バトルCLI（対戦専用）
- `data/` - ポケモン・技データ

## 🚀 使い方

### Debug CLI（推奨）

```bash
./debug-cli
```

**機能:**
- 🎮 バトルシミュレーション
- 📊 ポケモン情報確認
- ⚔️ 技情報確認
- 🧮 ダメージ計算機

**日本語検索機能:**
- ポケモンや技を選択する際、ひらがな・カタカナで検索できます
- 入力で絞り込めます（例: 'ぴかちゅう' → 'pikachu'）

### Battle CLI

```bash
./battle-cli
```

**機能:**
- 3vs3のバトルシミュレーション
- AIとの対戦
- 詳細モード（技を自分で選択）

## ⚡ ヒント

- Debug CLIの技選択画面で、`/moves` コマンドで技の詳細を確認できます
- Battle CLIでは詳細モードで技を選べます（起動時に選択）
- ローマ字で検索できるので、「10まんボルト」は「10man」で検索可能

## 🐛 トラブルシューティング

### macOSで「開発元が未確認」エラーが出る場合

1. システム設定 → プライバシーとセキュリティ
2. 「このまま開く」をクリック

または、ターミナルで以下を実行:

```bash
xattr -d com.apple.quarantine debug-cli
xattr -d com.apple.quarantine battle-cli
```

### 実行権限がない場合

```bash
chmod +x debug-cli battle-cli
```

## 📝 データについて

- `data/species.json` - ポケモンデータ
- `data/moves.json` - 技データ
- `data/learnsets.json` - 習得技データ

データは実行可能ファイルに埋め込まれているので、`data/`フォルダは削除しても動作します。

---

楽しんでください！ 🎉
EOF

# ZIP圧縮
echo "🗜️  ZIP圧縮中..."
zip -r nikopoke-cli-macos.zip "$DIST_DIR"

echo ""
echo "✅ 完成！"
echo ""
echo "📁 配布ファイル:"
echo "   - nikopoke-cli-macos.zip (友達に渡すファイル)"
echo ""
echo "📦 中身:"
ls -lh "$DIST_DIR"
echo ""
echo "💡 使い方:"
echo "   1. nikopoke-cli-macos.zip を友達に送る"
echo "   2. 解凍して ./debug-cli を実行"
echo ""
